...
export default function StudentTopicDetails() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      ...
      unsubscribeProfile = onSnapshot(doc(db, "users", user.uid), (snapshot) => {
        setStudentProfile((snapshot.data() as StudentProfile | undefined) ?? {});
      }, (error) => console.error("Failed to load student profile:", error));
      unsubscribeRequest = onSnapshot(doc(db, "joinRequests", `${id}_${user.uid}`), (snapshot) => {
        setRequestStatus(snapshot.exists() ? snapshot.data().status as JoinRequestStatus : null);
  ...
  const handleJoinRequest = async () => {
    ...
    setSubmitting(true);
    try {
      await submitJoinRequest(topic, user.uid, { ...studentProfile, email: studentProfile.email || user.email || "" }, requestMessage);
  ...
  const buttonLabel = requestStatus === "pending" ? "Request pending" : requestStatus === "accepted" ? "You joined this team" : requestStatus === "rejected" ? "Request declined" : "Join team";
  ...
  return <DashboardLayout role="student"><div className="mx-auto max-w-5xl px-2 sm:px-0">
    ...
    <div className="grid gap-6 lg:grid-cols-[1.55fr_.8fr]"><main className="space-y-6"><section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-[#2A2A2A] dark:bg-[#181818] sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300">{topic.category}</span><h1 className="mt-4 text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">{topic.title}</h1></div><div className="rounded-xl bg-emerald-50 px-3 py-2 text-right dark:bg-emerald-950/30"><p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{match.score}%</p><p className="text-[10px] font-medium text-emerald-700/70 dark:text-emerald-300/70">skill match</p></div></div><p className="mt-6 whitespace-pre-line text-sm leading-7 text-slate-600 dark:text-slate-300">{topic.description}</p><div className="mt-7 grid gap-4 border-t border-slate-100 pt-6 sm:grid-cols-2 dark:border-[#2A2A2A]"><Detail icon={CalendarDays} label="Application deadline" value={formatDeadline(topic.applicationDeadline)} /><Detail icon={Users} label="Maximum team size" value={`${topic.maxTeamSize} student${topic.maxTeamSize === 1 ? "" : "s"}`} /><Detail icon={Users} label="Current team" value={teamFull ? `${teamMemberCount}/${topic.maxTeamSize} members · Full` : `${teamMemberCount}/${topic.maxTeamSize} members · ${topic.maxTeamSize - teamMemberCount} spot${topic.maxTeamSize - teamMemberCount === 1 ? "" : "s"} left`} /></div></section>
      ...
      <section className="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-6 dark:border-indigo-500/20 dark:bg-indigo-500/5"><h2 className="font-bold text-slate-900 dark:text-white">Interested in this topic?</h2><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{teamFull ? "This team has reached its maximum size." : "Send your profile and skills to the supervisor for review."}</p>{!requestStatus && !teamFull && <div className="mt-4"><label htmlFor="join-message" className="text-xs font-semibold text-slate-700 dark:text-slate-200">Message to supervisor <span className="font-normal text-slate-500">(optional)</span></label><textarea id="join-message" value={requestMessage} onChange={(event) => setRequestMessage(event.target.value)} maxLength={400} rows={4} placeholder="Briefly explain why you are interested in this research topic..." className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-indigo-500 dark:border-slate-700 dark:bg-[#121212] dark:text-white" /><p className="mt-1 text-right text-[11px] text-slate-500">{requestMessage.length}/400</p></div>}{requestStatus === "pending" ? <button type="button" disabled={cancelling} onClick={handleCancelRequest} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-rose-200 bg-white px-4 py-3 text-sm font-semibold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-rose-500/30 dark:bg-transparent dark:hover:bg-rose-500/10">{cancelling ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{cancelling ? "Cancelling..." : "Cancel request"}</button> : <button type="button" disabled={Boolean(requestStatus) || submitting || teamFull} onClick={handleJoinRequest} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-50">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : requestStatus === "accepted" ? <CheckCircle2 className="h-4 w-4" /> : <Send className="h-4 w-4" />}{teamFull ? "Team is full" : submitting ? "Sending request..." : buttonLabel}</button>}</section></aside></div>
  </div></DashboardLayout>;
}
