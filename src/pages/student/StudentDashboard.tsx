...
export default function StudentDashboard() {
  ...
  return <DashboardLayout role="student"><div className="mx-auto max-w-5xl px-2 sm:px-0">
    ...
    <div className="mb-12 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <StatCard icon={FolderKanban} value={String(topics.length).padStart(2, "0")} label="Published topics" trend="Live from Firestore" />
    ...
    {loading ? <div className="flex h-64 items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-blue-600" /></div> : <>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white dark:border-[#2A2A2A] dark:bg-[#181818]"><SectionHeader title="Your research profile" description="This information powers topic matching." /><div className="space-y-5 p-6 text-sm"><ProfileField label="Department" value={profile.department || "Not added"} /><ProfileField label="CGPA" value={profile.cgpa || "Not added"} /><ProfileField label="Research interests" value={profile.researchInterests || "Add interests in your profile"} /><div><p className="text-xs text-slate-500 dark:text-slate-400">Skills</p><div className="mt-2 flex flex-wrap gap-2">{profile.skills?.length ? profile.skills.map((skill) => <span key={skill} className="rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{skill}</span>) : <span className="text-sm text-slate-500 dark:text-slate-400">Add skills to receive better matches.</span>}</div><form onSubmit={handleAddSkill} className="mt-4 flex gap-2"><input value={skillInput} onChange={(event) => setSkillInput(event.target.value)} placeholder="Add a skill, e.g. Python" className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-blue-500 dark:border-slate-700 dark:bg-[#121212] dark:text-white" /><button type="submit" disabled={savingSkill || !skillInput.trim()} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50">{savingSkill ? "Adding..." : "Add skill"}</button></form>{skillError && <p className="mt-2 text-xs text-rose-600 dark:text-rose-400">{skillError}</p>}</div></div></section></div>
    </>}</div></DashboardLayout>;
}
...
function RecentTopic({ topic }: { topic: ResearchTopic }) { return <div className="flex items-center gap-4 px-6 py-5"><div className="h-2.5 w-2.5 rounded-full bg-blue-500 dark:bg-[#3B82F6]" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-900 dark:text-white">{topic.title}</p><p className="mt-1 truncate text-xs text-slate-500 dark:text-slate-400">{topic.category} · {topic.supervisorName || "Unknown supervisor"}</p></div><span className="text-xs text-slate-400">{formatDeadline(topic.applicationDeadline)}</span></div>; }
