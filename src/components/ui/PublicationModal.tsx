...
import { extractPaperMetadataPDF } from "@/lib/ai";
import { generateAPACitation, generateIEEECitation } from "@/utils/citationGenerator";
...
export function PublicationModal({ isOpen, onClose, onSave, editingPublication, defaultAuthors }: PublicationModalProps) {
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      ...
      setAiError("");
      const metadata = await extractPaperMetadataPDF(selectedFile);
      
      if (metadata.title) setTitle(metadata.title);
      if (metadata.authors && metadata.authors.length > 0) setAuthors(metadata.authors);
      if (metadata.venue) setVenue(metadata.venue);
      if (metadata.publicationDate) setPublicationDate(metadata.publicationDate);
      if (metadata.keywords && metadata.keywords.length > 0) setKeywords(metadata.keywords);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      data.generatedCitations = {
        apa: generateAPACitation(data),
        ieee: generateIEEECitation(data),
      };
          ...
          <form id="publication-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Abstract / Summary</label>
                <button
                  type="button"
                  onClick={() => setShowAiInput((prev) => !prev)}
                  className="flex items-center gap-1.5 rounded-lg bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 transition-colors hover:bg-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-300 dark:hover:bg-indigo-500/30"
                >
              ...
              {showAiInput && (
                <div className="mb-3 space-y-3 rounded-xl border border-indigo-100 bg-indigo-50/50 p-4 dark:border-indigo-500/20 dark:bg-indigo-500/5">
                  <p className="text-xs font-medium text-slate-600 dark:text-slate-400">
                    Upload the PDF of the research paper below. The AI will read the file, extract the metadata (Title, Authors, Date, Venue, Keywords), and generate a summary to automatically fill out this form!
                  </p>
                  <input
                    type="file"
                    ...
                    className="block w-full text-sm text-slate-500 file:mr-4 file:cursor-pointer file:rounded-xl file:border-0 file:bg-indigo-600 file:px-4 file:py-2.5 file:text-sm file:font-semibold file:text-white hover:file:bg-indigo-700 dark:file:bg-indigo-600 dark:hover:file:bg-indigo-700"
                  />
                  {aiError && <p className="text-xs font-medium text-rose-500">{aiError}</p>}
                  <div className="flex justify-end">
                    <button
                      type="button"
                      onClick={handleGenerateSummary}
                      disabled={isGeneratingSummary || !selectedFile}
                    ...
                    >
                      {isGeneratingSummary ? <><Loader2 className="h-4 w-4 animate-spin" /> Extracting Metadata & Summarizing...</> : "Auto-Fill & Summarize from PDF"}
                    </button>
                  </div>
                </div>
              )}
