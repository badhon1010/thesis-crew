...
export function StudentNotifications() {
  ...
  return <div className="relative"><button type="button" aria-label="Notifications" onClick={() => setIsOpen((open) => !open)} className="relative rounded-xl border border-slate-200 p-2.5 text-slate-600 transition hover:bg-slate-50 dark:border-[#2A2A2A] dark:text-slate-300 dark:hover:bg-[#181818]"><Bell className="h-5 w-5" />{unreadCount > 0 && <span className="absolute -right-1 -top-1 flex min-h-5 min-w-5 items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold text-white">{unreadCount > 9 ? "9+" : unreadCount}</span>}</button>{isOpen && <div className="absolute right-0 top-12 z-50 w-80 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-[#2A2A2A] dark:bg-[#181818]"><div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-[#2A2A2A]"><p className="text-sm font-bold text-slate-900 dark:text-white">Notifications</p><button type="button" disabled={markingRead || unreadCount === 0} onClick={markAllRead} className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 disabled:opacity-40 dark:text-indigo-400">{markingRead ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCheck className="h-3.5 w-3.5" />}Mark all read</button></div><div className="max-h-96 overflow-y-auto">{notifications.length ? notifications.slice(0, 8).map((notification) => <Link key={notification.id} onClick={() => { void markRead(notification); setIsOpen(false); }} to={`/student/research-topics/${notification.topicId}`} className={`block border-b border-slate-100 px-4 py-3 transition hover:bg-slate-50 dark:border-[#2A2A2A] dark:hover:bg-[#222222] ${notification.read ? "" : "bg-indigo-50/70 dark:bg-indigo-500/10"}`}><p className="text-sm font-semibold text-slate-900 dark:text-white">{notification.title}</p><p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-300">{notification.message}</p></Link>) : <p className="px-4 py-8 text-center text-sm text-slate-500">You are all caught up.</p>}</div></div>}</div>;
}
